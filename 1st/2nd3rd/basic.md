[前に戻る](./readme.md) [次にすすむ](./basic2.md)
## 目次
<!-- TOC -->
...
int++：1を足す（次は同値：`i++`，`i = i + 1`，`i += 1`）
- デクリメント

int--：1を引く（次は同値：`i--`，`i = i - 1`，`i -= 1`）
...
i = 0
```
- 今回ではdeferを実行した時点ではiは0だったため、`i = 0`だったときの内容が出てきたのです。

**Stacking defers**

関数内でdeferを複数回実行したとき、関数終了後に呼び出されるdefer節は実行した逆順になる。
