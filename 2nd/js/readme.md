...
- [第一回](#第一回)
    ...
    - [型変換](#型変換)
    - [演算子](#演算子)
            - [補足 - 演算子の優先順位 -](#補足---演算子の優先順位--)
    - [補足 - NaN -](#補足---nan--)
        - [補足 - JavaScriptに於ける同一性 -](#補足---javascriptに於ける同一性--)
    - [function宣言](#function宣言)
    - [巻き上げ](#巻き上げ)
    - [参考](#参考)
...
このように、varは関数またはそのファイル内がスコープになります。また、letではブロックがスコープになります。varを下手に用いると、予期しない動作となる場合があるので、基本的にはletを用いると良いでしょう。
...
console.log("parseFloat: "+parseFloat(x));// => 5.1
...
JavaScriptの`+`演算子は左から結合されていくため、このような出力になります。気をつけましょう。

#### 補足 - 演算子の優先順位 -
先程は`+`演算子のみに言及しましたが、他の演算子を含めた演算を行うときの計算順序には更にこまかい規則があります。興味があれば次を参照すると良いでしょう。

[演算子の優先順位 - JavaScript](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Operator_Precedence)

## 補足 - NaN -

これは、非数値(Not a Number)であることを示す、Number型の値です。この値の扱いは少し面倒です。この数は次の性質を持ちます。
...
しかし、場合によって、これは嬉しくない挙動となる可能性を秘めています。もし、違う型の場合弾いてほしい時は、論理演算子`===`もしくは、`!==`を用いるべきです。ほとんどの場合、`==`、`!=`は用いません。覚えておきましょう。
